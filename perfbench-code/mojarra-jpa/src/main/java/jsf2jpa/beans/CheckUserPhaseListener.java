...
public class CheckUserPhaseListener implements PhaseListener {
    ...
    public void afterPhase(PhaseEvent pe)
    {
        ...
        {
            //BookingSession session = pe.getFacesContext().getApplication().
            //    evaluateExpressionGet(pe.getFacesContext(), "#{"+BookingSession.NAME+"}", 
            //        BookingSession.class);
            
            BookingSession session = (BookingSession) pe.getFacesContext().
                    getExternalContext().getSessionMap().get(BookingSession.NAME);
